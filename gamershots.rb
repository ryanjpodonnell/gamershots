...
post '/play' do
  unless params.empty?
    ...
    platforms_query_string = "AND platforms LIKE ANY (VALUES#{platforms.map { |value| "('%#{value}%')" }.join(", ")})" if platforms
    publishers_query_string = "AND publishers LIKE ANY (VALUES#{publishers.map { |value| "('%#{value}%')" }.join(", ")})" if publishers
